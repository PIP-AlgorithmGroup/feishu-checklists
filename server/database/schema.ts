...
export const checklist = pgTable("checklist", {
  ...
  checklistKey: varchar("checklist_key", { length: 100 }).notNull().unique(),
   ...
   * @type { string[] }
   */
  processedEventIds: jsonb("processed_event_ids").notNull().default('[]'),
  ...
  uniqueIndex("checklist_checklist_key_key").on(table.checklistKey),
]);
